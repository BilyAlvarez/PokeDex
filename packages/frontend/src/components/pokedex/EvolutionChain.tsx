...
export function EvolutionChain({ evolutions }: EvolutionChainProps) {
  if (evolutions.length === 0) {
    return <p className="text-sm text-gray-500 italic">No evolutions</p>
  }

  return (
    <div className="space-y-3">
      {evolutions.map(evo => (
        <div key={evo.id} className="flex items-center gap-3 bg-[#E8E0D0] rounded-lg p-3">
          {evo.evolvesTo.spriteUrl && (
            <img src={evo.evolvesTo.spriteUrl} alt={evo.evolvesTo.name} className="w-12 h-12 object-contain" />
          )}
          <div className="flex-1">
            <p className="font-semibold capitalize text-[#2D2D2D]">{evo.evolvesTo.name}</p>
